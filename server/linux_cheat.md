...
```
NR: 現在の入力行の数（行番号）を表す。
NF: 現在の入力行のフィールド（列）の数を表す。
$0: 現在の入力行全体を表す。
$1, $2, $3, ...: 現在の入力行の1番目、2番目、3番目のフィールド（列）を表す。フィールドの番号は1から始まる。
FS: 入力フィールド（列）の区切り文字を表す。
RS: 入力レコード（行）の区切り文字を表す。
OFS: 出力フィールド（列）の区切り文字を表す。
ORS: 出力レコード（行）の区切り文字を表す。
FILENAME: 現在処理中のファイルの名前を表す。
FNR: 現在処理中のファイルにおける行番号を表す。
```

・特殊なパターン

```
BEGIN: パターンは入力の終了時に一度だけ実行されるブロックを定義する。
END: 入力の開始時に一度だけ実行されるブロックを定義する。
```
...
・if (条件) {} else {}
awk '{ if ($1 > 10) print "数値が10より大きいです"; else print "数値が10以下です" }' file.txt

・while (条件) {}
awk '{ while ($1 > 0) { print $1--; } }' sample.txt
